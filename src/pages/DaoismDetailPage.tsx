...
import { getDaoismChapterById, getDaoismCategoryById } from '../data/daoism';
...
export default function DaoismDetailPage() {
  const { category, id } = useParams<{ category: string; id: string }>();
  const navigate = useNavigate();

  const chapter = id ? getDaoismChapterById(id) : undefined;
    ...
    return (
      <InkBackground>
        <div className="min-h-dvh flex flex-col items-center justify-center gap-4 px-6 pb-24">
          <p className="text-ink-light text-lg">文章未找到</p>
  ...
  const allChapters = cat.chapters;
      ...
      <div className="max-w-lg mx-auto px-4 sm:px-6 py-6 sm:py-8 pb-24">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="text-ink-pale text-xs">{cat.title}</span>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            <span className="text-sm">💡</span>
            <span className="text-cinnabar text-xs font-bold tracking-wider">生活体悟</span>
          </div>
        ...
        <div className="flex justify-between mt-8">
          {prevChapter ? (
            <button
              onClick={() => navigate(`/daojia/${cat.id}/${prevChapter.id}`)}
          ...
          {nextChapter ? (
            <button
              onClick={() => navigate(`/daojia/${cat.id}/${nextChapter.id}`)}
